...
Timeline.CompactEventPainter.prototype.paint = function() {
    ...
    var stackConcurrentPreciseInstantEvents = "stackConcurrentPreciseInstantEvents" in this._params && this._params.stackConcurrentPreciseInstantEvents;
...
Timeline.CompactEventPainter.prototype.paintCompositePreciseInstantEvents = function(events, metrics, theme, highlightIndex) {
    ...
    var self = this;
    var clickHandler = function(elmt, domEvt, target) {
        return self._onClickCompositePreciseInstantEvent(result.iconElmtData.elmt, domEvt, events);
    };
...
Timeline.CompactEventPainter.prototype.paintStackedPreciseInstantEvents = function(events, metrics, theme, highlightIndex) {
    ...
    for (var i = 0; i < events.length; i++) {
        ...
        //Modified by Uday (+i*horizontalIncrement)
        //record.top = totalLabelHeight;
        record.top = i*horizontalIncrement;        
        records.push(record);
    ...
    //Added by Uday (onStackMouseOut for the entire stack)
    var onStackMouseOut = function(domEvt) {
    ...
    //Added by Uday (onMouseOut for the entire stack)
    /*
    var onStackMouseOver = function(domEvt) {
        try {
            var childNodes = iconStackDiv.firstChild.childNodes;            
            for (var i = 0; i < childNodes.length; i++) {
                ...
                var child = childNodes[i];
                child.style.zIndex = childNodes.length - i;
                child.style.top = record.iconLeft + "px";
                child.style.left = record.top + "px";
            }
        } catch (e) {
        }
    }; 
    */
    ...
    //Modified by Uday (+25 pixels)
    iconStackDiv.style.width = (rightIconEdge+25) + "px";
    iconStackDiv.style.height = (totalIconHeight+25) + "px";
    iconStackDiv.innerHTML = "<div style='position: relative'></div>";
    //iconStackDiv.onmousever = onStackMouseOver;
    iconStackDiv.onmouseout = onStackMouseOut;
    ...
    var paintEvent = function(index) {
        ...
        /*
        if(index==0) {
        	iconDiv.onmouseout = onStackMouseOut;
        	iconDiv.onmouseover = onStackMouseOver;
        } else {
        	iconDiv.onmouseover = onMouseOver;        	
        }
        */
...
Timeline.CompactEventPainter.prototype._paintEventLabel = function(commonData, labelData, left, top, width, height, theme) {
    ...
    //Modified by Uday (tinytext)
    labelDiv.className = 'timeline-event-label tinytext';
...
Timeline.CompactEventPainter.prototype._onClickCompositePreciseInstantEvent = function(icon, domEvt, events) {
